...
- (instancetype)initWithOrigin:(CGPoint)origin andHeight:(CGFloat)height inView:(UIView *)view{
    ...
    if (self) {
        ...
        _leftTableView.sectionHeaderHeight=0.0001;
        ...
        _rightTableView.sectionHeaderHeight=0.0001;
        if (@available(iOS 11.0, *)) {
...
- (void)animateLeftTableView:(UITableView *)leftTableView rightTableView:(UITableView *)rightTableView show:(BOOL)show complete:(void(^)(void))complete {
    if (@available(iOS 11.0, *)) {
        if ([leftTableView respondsToSelector:@selector(setContentInsetAdjustmentBehavior:)]) {
            leftTableView.contentInsetAdjustmentBehavior =UIScrollViewContentInsetAdjustmentNever;
        }
    }
    if (@available(iOS 11.0, *)) {
        if ([rightTableView respondsToSelector:@selector(setContentInsetAdjustmentBehavior:)]) {
